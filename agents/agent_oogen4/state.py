...
class State:
    ...
    def create_plan_step(self, version: Optional[int] = None) -> str:
        """
        Create a plan step directory.
        ...
        self.current_plan_step = plan_step_dir
        return plan_step_dir
    ...
    def list_plan_versions(self) -> List[str]:
        ...
        return [d for d in os.listdir(self.run_dir) if d.startswith("plan_v") and os.path.isdir(os.path.join(self.run_dir, d))]
    ...
    def current_plan_data(self) -> Dict[str, Optional[str]]:
        ...
        Returns:
            ...
            - "plan_steps": A collection of plan step data, including all iterations
        """
        if self.current_plan_version is None:
            raise ValueError("No plan version selected")
        ...
        if os.path.exists(plan_step_dir):
            ...
            if os.path.exists(plan_step_text_path):
                with open(plan_step_text_path, "r") as f:
                    data["plan_step"]["plan_step_text"] = f.read()
            ...
            if os.path.exists(plan_step_result_path):
                with open(plan_step_result_path, "r") as f:
                    data["plan_step"]["plan_step_result"] = f.read()

            # List all iterations in the plan step
            data["plan_step"]["iterations"] = self._get_current_plan_step_iterations_data()

        return data

    def _get_current_plan_step_iterations_data(self) -> List[Dict[str, Optional[str]]]:
        """
        ...
        """
        if self.current_plan_step is None:
            # raise ValueError("No plan step selected")
            return []

        iterations_data = []
        
        # Loop through all iteration directories
        for iteration_folder in sorted(os.listdir(self.current_plan_step)):
            if iteration_folder.startswith("iteration_"):
                iteration_number = int(iteration_folder.split("_")[1])
                iteration_dir = os.path.join(self.current_plan_step, iteration_folder)
                validation_dir = os.path.join(iteration_dir, "validation")
    ...
    def __str__(self) -> str:
        ...
        return f"State(run_dir={self.run_dir}, current_plan_version={self.current_plan_version})"