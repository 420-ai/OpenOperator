    ...
    async def execute(self, actions_history: str) -> bool:
        ...
        user_message = {
            ...
            "content":  [
                {
                    "type": "text",
                    "text":  USER_MESSAGE.format(objective=plan_step, actions_history=actions_history)
                },
                {
                    "type": "image_url",
                    "image_url": {
                        "url": f"data:image/png;base64,{encode_image(screenshot_t1)}",
                    }
                },
                {
                    "type": "image_url",
                    "image_url": {
                        "url": f"data:image/png;base64,{encode_image(screenshot_t2)}",
                    }
                }
            ]
        }
