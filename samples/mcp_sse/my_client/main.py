...
async def start():
    ...
    # Use environment variable for server URL, defaulting to local address
        ...
        async with sse_client(server_url + "/sse") as streams:
            async with ClientSession(*streams) as session:
                ...
                for resource in resources_list_response.resources:
                    print(
                        f"Resource URI: {resource.uri}, Name: {resource.name}, MIMEType: {resource.mimeType}"
                    )
                ...
                stock_prompt = await session.get_prompt(
                    "stock-summary", arguments={"ticker": "AAPL", "days": "5"}
                )
