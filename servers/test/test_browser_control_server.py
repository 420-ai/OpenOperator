...
BASE_URL = "http://192.168.2.114:5051"
...
def test_get_local_storage(page_id):
    resp = requests.post(
        f"{BASE_URL}/browser/get_local_storage", json={"page_id": page_id}
    )
...
def test_execute_js(page_id, js_code):
    resp = requests.post(
        f"{BASE_URL}/browser/execute_js", json={"page_id": page_id, "js": js_code}
    )
...
def test_cdp(page_id, command, params={}):
    resp = requests.post(
        f"{BASE_URL}/browser/cdp",
        json={"page_id": page_id, "command": command, "params": params},
    )
...
def test_download_trace(trace_file):
    resp = requests.get(
        f"{BASE_URL}/browser/download_trace", params={"trace_file": trace_file}
    )
    if resp.status_code == 200:
